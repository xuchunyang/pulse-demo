...
const confirmingUserDeletion: Record<number, boolean> = reactive(
    props.users
        .map((user) => user.id)
        .reduce(
            (acc, id) => {
...
const editingUser: Record<number, boolean> = reactive(
    props.users
        .map((user) => user.id)
        .reduce(
            (acc, id) => {
                        ...
                        <table class="table-auto w-full border-collapse">
                            ...
                            <tbody>
                                <tr v-for="user in users" :key="user.id">
                                    <td
                                        class="border dark:border-gray-700 px-4 py-2"
                                    >
                                        {{ user.id }}
                                    </td>
                                    <td
                                        ...
                                        <div class="space-x-2">
                                            ...
                                            >
                                                <div class="p-6">
                                                    ...
                                                    <div
                                                        class="mt-2 whitespace-pre text-sm bg-gray-50 p-2"
                                                    >
                                                        {{ user }}
                                                    </div>
                                            ...
                                            <Modal
                                                ...
                                                "
                                                @close="
                                                    closeConfirmingUserDeletionModal(
                                                        user.id,
                                                    )
                                                "
                                            >
